...
import time
import random
# создаём новый объект — окно с игровым полем. В нашем случае переменная окна
# называется tk, и мы его сделали из класса Tk() — он есть в граф библиотеке
tk = Tk()
...
tk.resizable(0, 0)
# помещаем наше игровое окно выше остальных окон на компьютере, чтобы другие
# окна не могли его заслонить. Попробуйте :)
...
class Ball:
    ...
    def __init__(self, canvas, paddle, score, color, number):
        ...
        if number == 2:
            ...
            self.canvas.move(self.id, 255, 100)
            self.SPEED = 2
...
ball_2 = Ball(canvas, paddle, score, 'green', 2)
# пока шарик не коснулся дна
while not ball.hit_bottom and not ball_2.hit_bottom:
